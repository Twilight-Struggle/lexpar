...
fn lex(input: &str) -> Result<Vec<Token>, LexError> {
    ...
    while pos < input.len() {
        match input[pos] {
            ...
            b'*' => lex_a_token!(lex_mul(input, pos)),
            b'/' => lex_a_token!(lex_div(input, pos)),
            ...
            b => return Err(LexError::invalid_char(b as char, Loc(pos, pos+1))) // 文字を入れればその他扱いになる。Ocamlでいうwhenはifを使う
        }
    }
    Ok(tokens)
}
...
fn consume_byte(input:&[u8], pos: usize, b: u8) -> Result<(u8, usize), LexError> {
    if pos >= input.len() {
        Err(LexError::eof(Loc(pos, pos)))
    }
    else {
        if input[pos] != b {
            Err(LexError::invalid_char(input[pos] as char, Loc(pos, pos+1)))
        }
        else {
            Ok((b, pos+1))
        }
...
fn lex_mul(input: &[u8], start: usize) -> Result<(Token, usize), LexError> {
    consume_byte(input, start, b'*').map(|(_, end)| (Token::mul(Loc(start, end)), end))
}
fn lex_div(input: &[u8], start: usize) -> Result<(Token, usize), LexError> {
    consume_byte(input, start, b'/').map(|(_, end)| (Token::div(Loc(start, end)), end))
}
        ...
        match tokens.peek() {
            Some(_) => {
                let op = match op_parser(tokens) {
                    Ok(op) => op, // ここ で パース に 失敗 し た のは これ 以上 中 置 演算子 が ない という 意味
                    Err(_) => break
                };
...
where Tokens: Iterator<Item = Token>
{
    ...
    where Tokens: Iterator<Item = Token> {
        let op = tokens.peek().ok_or(ParseError::Eof).and_then(
            |tok| match tok.value {
...
where Tokens: Iterator<Item = Token>
{
    ...
    where Tokens: Iterator<Item = Token> {
        let op = tokens.peek().ok_or(ParseError::Eof).and_then(
            |tok| match tok.value {
                TokenKind::Mul => Ok(BinOp::mult(tok.loc.clone())),
                TokenKind::Div => Ok(BinOp::div(tok.loc.clone())),
                _ => Err(ParseError::NotOperator(tok.clone()))
            }
        )?;
        tokens.next();
